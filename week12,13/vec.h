...
public:
	...
	*/
	...
	//4.template 추가
	template <class ln> Vec(ln b, ln e) {
		create(b, e);
	}
	...
	//insert & assign 추가(Q12,Q13)
	template<class ln> iterator insert(iterator, ln, ln); // 12번문제
	void assign(iterator, iterator); //13번 문제 assign(할당/정의)
...
template <class T>
template <class ln>
typename Vec<T>::iterator Vec<T>::insert(iterator d, ln b, ln e) {
