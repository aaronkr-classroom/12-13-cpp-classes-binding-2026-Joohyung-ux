...
public:
	Nodeltr& operator++(); //한개 씩 증가 연산자
	Nodeltr& operator--(); //감소 연산자
	Nodeltr operator++(int); //next로 이동
	Nodeltr operator--(int); //prev로 이동
...
private:
	...
	void create(const_iterator, const_iterator); //create(begin, end)
...
void Lis<T>::push_back(const T& t) {
	Node<T>* node = new Node<T>(t);// t vaule
